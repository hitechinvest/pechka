...
(function (global) {
  ...
  /* Штукатурка: белёная стена с лёгкими потёками — для соборов и арок. */
  World.plasterTexture = function (color) {
    return canvasTexture(128, 128, function (g, w, h) {
      g.fillStyle = color || '#f0e9db';
      g.fillRect(0, 0, w, h);
      for (var i = 0; i < 900; i++) {
        g.fillStyle = Math.random() > 0.5 ? 'rgba(255,255,255,0.25)' : 'rgba(150,140,120,0.16)';
        g.fillRect(Math.random() * w, Math.random() * h, 2, 2);
      }
      for (var k = 0; k < 12; k++) {
        g.strokeStyle = 'rgba(140,130,110,0.14)';
        g.lineWidth = 1 + Math.random() * 2;
        g.beginPath();
        var xx = Math.random() * w;
        g.moveTo(xx, 0);
        g.lineTo(xx + (Math.random() - 0.5) * 20, h);
        g.stroke();
      }
    }, 2, 2);
  };
  ...
  World.facadeTexture = function (style) {
    return canvasTexture(256, 512, function (g, w, h) {
      ...
      if (style === 'sand') {
        ...
        // альпийское шале: белёный низ, тёмный брус, ставни и балкон.
        // Окон мало и они крупные — дом-то маленький.
        cols = 3; rows = 4;
        cw = w / cols; ch = h / rows;
        g.fillStyle = '#efe6d4';
        g.fillRect(0, 0, w, h);
        noiseOverlay(g, w, h, 1600, 'rgba(255,252,244,0.4)', 'rgba(150,136,110,0.22)');
        for (var rc = 0; rc < rows; rc++) {
          var yc = rc * ch;
          // венцы бруса
          g.fillStyle = 'rgba(108,74,44,0.85)';
          g.fillRect(0, yc + ch - 5, w, 5);
          g.fillStyle = 'rgba(140,100,62,0.5)';
          g.fillRect(0, yc + ch - 10, w, 2);
          for (var cc = 0; cc < cols; cc++) {
            var xc = cc * cw;
            // окно с белой рамой
            g.fillStyle = '#f7f4ec';
            g.fillRect(xc + 4, yc + 5, cw - 8, ch - 16);
            var litc = Math.random();
            g.fillStyle = litc > 0.72 ? '#ffdf9e' : '#5c6b74';
            g.fillRect(xc + 7, yc + 8, cw - 14, ch - 22);
            // ставни по бокам
            g.fillStyle = '#8f3a2c';
            g.fillRect(xc + 2, yc + 5, 4, ch - 16);
            g.fillRect(xc + cw - 6, yc + 5, 4, ch - 16);
            // ящик с геранью на подоконнике
            if (Math.random() > 0.45) {
              g.fillStyle = '#6b4a2a';
              g.fillRect(xc + 5, yc + ch - 15, cw - 10, 4);
              g.fillStyle = '#c9403a';
              for (var fx = xc + 6; fx < xc + cw - 6; fx += 5) {
                g.beginPath(); g.arc(fx, yc + ch - 16, 2.1, 0, Math.PI * 2); g.fill();
              }
            }
          }
        }
  ...
  /* Разметка трасс. Кольцо по городу — быстрое и плавное, альпийский
     перевал — серпантин с четырьмя шпильками и длинным спуском. */
  World.TRACKS = {
    ring: {
      width: 22,
      points: [
      ...
      hills: [
        { amp: 7.0, harm: 2 }, { amp: 4.0, harm: 3 }, { amp: 2.0, harm: 7 },
        { amp: 0.7, harm: 23 }, { amp: 0.3, harm: 37 }
      ],
      jumps: [
    ...
    /* Альпы: со старта уходим на запад, четыре шпильки серпантина вниз
       по склону, внизу траверс и длинный скоростной подъём по восточной
       стороне обратно к финишу. Между соседними петлями 80–90 метров,
    ...
    alps: {
      ...
      points: [
        ...
        [-20, -155], [85, -165],
        [145, -190], [140, -235], [75, -245],
        [-35, -250], [-140, -262],
        [-200, -290], [-190, -335], [-125, -345],
        [-20, -352], [90, -360],
        [160, -385], [200, -360], [215, -300],
  ...
  World.buildTrack = function (name) {
    var spec = World.TRACKS[name] || World.TRACKS.ring;
    var pts = spec.points.map(function (p) { return new THREE.Vector3(p[0], 0, p[1]); });
    var curve = new THREE.CatmullRomCurve3(pts, true, 'catmullrom', 0.5);

    var track = {
      curve: curve,
      layout: spec === World.TRACKS.alps ? 'alps' : (name || 'ring'),
    ...
    /* Крутизна поворота в плане: 1/R со знаком (плюс — влево).
       По ней считается снос на скорости и то, как тормозит компьютер. */
    track.turnAt = function (dist) {
      var d = 6;
      var a = this.tangentAt(dist - d);
      var b = this.tangentAt(dist + d);
  ...
  World.THEMES = {
    ...
    yoshkar: {
      ...
      facades: ['brick', 'haussmann', 'white', 'stripe'],
      facadeTints: [0xffffff, 0xe8c07a, 0xc98a6a, 0x9fbcd4],
      heights: [11, 17], capColor: 0x8f3a2e, roofs: 'gable',
      trees: 'spruce', treeCount: 86, dunes: false,
      weather: ['snow', 'rain'],
      people: 'mari', beast: 'elk',
      landmarks: 'yoshkar'
    },
    tokyo: {
    ...
    swiss: {
      ...
      trees: 'spruce', treeCount: 96, dunes: false, mountains: true,
      weather: ['snow', 'snow', 'rain'],
      people: 'swiss', beast: 'cow',
      landmarks: 'swiss'
    },
    paris: {
  ...
  World.buildCity = function (track, scene, theme) {
    ...
    // Где будут достопримечательности: считаем заранее, чтобы дома их обошли
    function spotAt(u, off) {
      var d = track.length * u;
      var p = track.pointAt(d);
      var n = track.sideAt(d);
    ...
    // тюремный двор стоит там же, где его ставит игра, — дома его обходят
    var J = spotAt(World.jailSpot(track).dist / track.length, 34);
    ...
    while (placed < 140 && tries < 2400) {
      ...
      var busy = false;
      for (var rz = 0; rz < reserved.length; rz++) {
        var sp = reserved[rz][0], rr2 = reserved[rz][1];
        if ((x - sp.x) * (x - sp.x) + (z - sp.z) * (z - sp.z) < rr2 * rr2) { busy = true; break; }
      }
      if (busy) continue;                                 // тут стоит достопримечательность

      var hmin = theme.heights[0], hspan = theme.heights[1];
      var h = hmin + Math.random() * Math.random() * hspan +
        (theme.roofs === 'gable' || near < 90 ? 0 : hmin * 0.6);
      ...
      if (theme.roofs === 'gable') {
        // у низких домов тайл фасада — примерно этаж на этаж
        World.scaleUV(geo, Math.max(1, Math.round(bw / 13)), Math.max(1, Math.round(h / 13)));
      } else {
      ...
      if (theme.roofs === 'gable') {
        // малоэтажный город: черепичная крыша шатром и печная труба
        var rh = 3 + Math.random() * 3.5;
        var roof = new THREE.ConeGeometry(1, rh, 4);
        roof.rotateY(Math.PI / 4);
        roof.applyMatrix4(new THREE.Matrix4().makeScale(bw * 0.82, 1, bd * 0.82));
        roof.translate(0, h / 2 + rh / 2, 0);
        roof.applyMatrix4(m);                    // тот же разворот, что и у дома
        capGeos.push(roof);
        if (Math.random() > 0.5) {
          var chim = new THREE.BoxGeometry(1.1, 3.4, 1.1);
          chim.translate(x + (Math.random() - 0.5) * bw * 0.35, h + rh * 0.4,
                         z + (Math.random() - 0.5) * bd * 0.35);
          capGeos.push(chim);
        }
    ...
    for (var b2 = 0; b2 < BUCKETS; b2++) {
      ...
      // в малоэтажных городах дома подкрашены, как на набережной Брюгге
      var tint = theme.facadeTints ? theme.facadeTints[b2 % theme.facadeTints.length] : 0xffffff;
      var mat = glassy
        ? new THREE.MeshPhongMaterial({ map: tex, emissiveMap: tex, emissive: 0x1a1712,
            specular: 0x6a7c8c, shininess: 42, color: tint })
    ...
    // Достопримечательности: у каждого города свои
    World.burjPosition = new THREE.Vector3(A.x, 0, A.z);

    if (theme.landmarks === 'dubai') {
      World.buildBurj(group, A.x, A.z);
      World.buildPalace(group, B.x, B.z, B.face);
    } else if (theme.landmarks === 'moscow') {
      World.buildKremlin(group, A.x, A.z, A.face);
      World.buildCathedral(group, B.x, B.z, B.face, 'basil');
      World.buildPalace(group, D.x, D.z, D.face);              // усадьба вдалеке
    } else if (theme.landmarks === 'yoshkar') {
      // Йошкар-Ола: набережная Брюгге, Царевококшайский кремль,
      // Благовещенский собор и колесо обозрения
      World.buildFlemishRow(group, A.x, A.z, A.face);
      World.buildKremlin(group, B.x, B.z, B.face);
      World.buildCathedral(group, C.x, C.z, C.face, 'blue');
      World.buildFerrisWheel(group, D.x, D.z, D.face);
      World.buildFlemishRow(group, E.x, E.z, E.face);           // вторая набережная
    } else if (theme.landmarks === 'tokyo') {
      World.buildTokyoTower(group, A.x, A.z);
      ...
      World.buildFlemishRow(group, D.x, D.z, D.face);
    }
    ...
    // Горы вокруг альпийской трассы
    if (theme.mountains) World.buildMountains(group, track);
  ...
  /* Вся одежда пешеходов лежит в одной текстуре-палитре 8×8 клеток:
     клетка либо залита цветом, либо расписана узором (тельняшка, марийская
     вышивка, кимоно). Тогда фигурка склеивается в три сетки и почти
     не стоит draw call'ов. */
  var PED_CELL = 32, PED_GRID = 8;
  var PED_CELLS = [
    // 0..7 — кожа, волосы, обувь
    { fill: '#e9bd97' }, { fill: '#cd935f' }, { fill: '#8d5a3b' },
    { fill: '#241d16' }, { fill: '#6b4b2a' }, { fill: '#c9b287' }, { fill: '#8d8d8d' },
    { fill: '#1a1a1e' },
    // 8..15 — обычная городская одежда
    { fill: '#e14b4b' }, { fill: '#3f7fd8' }, { fill: '#39b06a' }, { fill: '#f0c23c' },
    { fill: '#8a5cc4' }, { fill: '#eae6de' }, { fill: '#2c3550' }, { fill: '#6b5f4a' },
    // 16..23 — национальное
    { fill: '#f6f4ee' },                                       // 16 кандура, марийский лён
    { fill: '#17171c' },                                       // 17 абайя
    { pattern: 'stripes', a: '#f2f4f8', b: '#1d3566' },        // 18 тельняшка
    { pattern: 'embroidery', a: '#f7f3e7', b: '#c0392b' },     // 19 марийская вышивка
    { pattern: 'floral', a: '#243a72', b: '#dfe6f5' },         // 20 кимоно индиго
    { pattern: 'floral', a: '#c9718f', b: '#fff0f4' },         // 21 кимоно розовое
    { pattern: 'dots', a: '#b3242a', b: '#f4e9d8' },           // 22 русский платок
    { fill: '#6a4a30' },                                       // 23 шуба и шапка-ушанка
    // 24..31 — мелочи
    { fill: '#c9433c' }, { fill: '#2f6f5a' }, { fill: '#d8c07a' }, { fill: '#3a2f26' },
    { fill: '#9b1f2a' }, { fill: '#f0e4c8' }, { fill: '#2b2b30' }, { fill: '#d8d4c8' }
  ];

  var PED_PALETTE = null;
  function pedPalette() {
    if (PED_PALETTE) return PED_PALETTE;
    var size = PED_CELL * PED_GRID;
    var c = document.createElement('canvas');
    c.width = size; c.height = size;
    var g = c.getContext('2d');
    g.fillStyle = '#888888';
    g.fillRect(0, 0, size, size);
    for (var i = 0; i < PED_CELLS.length; i++) {
      var cell = PED_CELLS[i];
      var x = (i % PED_GRID) * PED_CELL, y = Math.floor(i / PED_GRID) * PED_CELL;
      g.save();
      g.beginPath();
      g.rect(x, y, PED_CELL, PED_CELL);
      g.clip();
      g.fillStyle = cell.fill || cell.a;
      g.fillRect(x, y, PED_CELL, PED_CELL);
      if (cell.pattern === 'stripes') {
        g.fillStyle = cell.b;
        for (var s = 0; s < PED_CELL; s += 8) g.fillRect(x, y + s, PED_CELL, 4);
      } else if (cell.pattern === 'embroidery') {
        g.fillStyle = cell.b;
        g.fillRect(x, y + 4, PED_CELL, 3);
        g.fillRect(x, y + PED_CELL - 8, PED_CELL, 3);
        for (var e = 0; e < PED_CELL; e += 6) {      // ромбики марийского узора
          g.beginPath();
          g.moveTo(x + e + 3, y + 12); g.lineTo(x + e + 6, y + 16);
          g.lineTo(x + e + 3, y + 20); g.lineTo(x + e, y + 16);
          g.closePath(); g.fill();
        }
      } else if (cell.pattern === 'floral') {
        g.fillStyle = cell.b;
        for (var f = 0; f < 9; f++) {
          var fx = x + 4 + (f % 3) * 11 + (Math.floor(f / 3) % 2) * 4;
          var fy = y + 5 + Math.floor(f / 3) * 10;
          for (var pt = 0; pt < 5; pt++) {           // цветок из пяти лепестков
            var a = pt * Math.PI * 0.4;
            g.beginPath();
        ...
        for (var d = 0; d < 16; d++) {
          g.beginPath();
          g.arc(x + 4 + (d % 4) * 8, y + 4 + Math.floor(d / 4) * 8, 2.2, 0, Math.PI * 2);
          g.fill();
        }
      }
      g.restore();
    ...
    PED_PALETTE.minFilter = THREE.LinearFilter;
    PED_PALETTE.magFilter = THREE.LinearFilter;
    PED_PALETTE.encoding = THREE.sRGBEncoding;
    return PED_PALETTE;
  }

  /* Красим геометрию «клеткой номер i»: все её UV сводим в центр клетки. */
  function paint(geo, i) {
    var uv = geo.attributes.uv;
    var cx = ((i % PED_GRID) + 0.5) / PED_GRID;
    var cy = 1 - (Math.floor(i / PED_GRID) + 0.5) / PED_GRID;
    for (var k = 0; k < uv.count; k++) uv.setXY(k, cx, cy);
    uv.needsUpdate = true;
    return geo;
  }

  /* То же, но с узором: родные UV раскладываем внутри клетки, чтобы
     полоски и вышивка легли по ткани. */
  function paintPattern(geo, i, rx, ry) {
    var uv = geo.attributes.uv;
    var col = i % PED_GRID, row = Math.floor(i / PED_GRID);
    var pad = 0.5 / (PED_CELL * PED_GRID);
    for (var k = 0; k < uv.count; k++) {
      var u = ((uv.getX(k) * (rx || 1)) % 1 + 1) % 1;
      var v = ((uv.getY(k) * (ry || 1)) % 1 + 1) % 1;
      uv.setXY(k,
        (col + pad + u * (1 - 2 * pad)) / PED_GRID,
        1 - (row + pad + (1 - v) * (1 - 2 * pad)) / PED_GRID);
    }
    uv.needsUpdate = true;
    return geo;
  }
  ...
  /* Пешеход в национальном костюме своего города.
     style: dubai | moscow | mari | tokyo | paris (или city — обычный горожанин).
     Ноги — отдельные сетки, чтобы шагали. */
  World.buildPedestrian = function (style, kind) {
    style = style || (World.theme && World.theme.people) || 'city';
    kind = kind || (Math.random() < 0.5 ? 'man' : 'woman');
    var g = new THREE.Group();
    var mat = new THREE.MeshLambertMaterial({ map: pedPalette() });
    var skin = pick([0, 0, 1, 1, 2]);
    var hair = pick([3, 3, 4, 5]);
    var body = [];

    // что на человеке: верх, низ, штанина, обувь и головной убор
    var top = pick([8, 9, 10, 11, 12, 13]);
    var bottom = pick([14, 15, 30]);
    var legColor = bottom;
    var shoe = 7;
    var robe = 0;                 // длинная одежда: 0 — нет
    var pattern = 0;              // узорная клетка для верха

    if (style === 'dubai') {
      if (kind === 'man') { robe = 16; top = 16; }
      else { robe = 17; top = 17; }
      legColor = robe;
      shoe = robe === 17 ? 7 : 29;
    } else if (style === 'moscow') {
      top = pick([23, 14, 30, 24]);            // шубы и пальто
      bottom = pick([14, 30, 27]);
      legColor = bottom;
      shoe = 7;
    } else if (style === 'mari') {
      robe = 19; top = 19; pattern = 19;       // белый лён с красной вышивкой
      legColor = kind === 'man' ? 30 : 19;
      shoe = 27;
    } else if (style === 'tokyo') {
      robe = kind === 'woman' ? pick([21, 20]) : 20;
      top = robe; pattern = robe;
      legColor = robe;
      shoe = 27;
    } else if (style === 'paris') {
      top = 18; pattern = 18;                  // тельняшка
      bottom = pick([14, 30, 27]);
      legColor = bottom;
      shoe = 7;
    } else if (style === 'swiss') {
      // трахтен: белая рубаха, тёмные штаны на лямках, у женщин — дирндль
      top = 16;
      bottom = kind === 'woman' ? 24 : 27;
      legColor = kind === 'woman' ? 16 : 27;
      shoe = 27;
    } else if (style === 'guard') {
      kind = 'man';
    ...
    var legs = [];
    var barelegs = (kind === 'woman' && style === 'city' && Math.random() < 0.5);
    for (var s = -1; s <= 1; s += 2) {
      ...
      parts.push(paint(shin, barelegs ? skin : legColor));
      var shoeGeo = new THREE.BoxGeometry(0.15, 0.1, 0.28);
      shoeGeo.translate(0, -0.83, 0.05);
      parts.push(paint(shoeGeo, shoe));
      if (style === 'tokyo') {                  // гэта: деревянная подошва на зубьях
        var soleA = new THREE.BoxGeometry(0.16, 0.06, 0.3);
        soleA.translate(0, -0.91, 0.05);
        parts.push(paint(soleA, 26));
      }
    ...
    // --- длинная одежда или юбка
    if (robe) {
      var rl = style === 'dubai' ? 1.16 : (style === 'tokyo' ? 1.0 : 0.92);
      var gown = new THREE.CylinderGeometry(0.2, style === 'dubai' ? 0.36 : 0.3, rl, 14);
      gown.translate(0, 0.62 + rl / 2 - 0.5, 0);
      body.push(pattern ? paintPattern(gown, pattern, 2, 2) : paint(gown, robe));
    } else if (kind === 'woman') {
      var skirt = new THREE.CylinderGeometry(0.19, 0.3, 0.5, 12);
      skirt.translate(0, 0.86, 0);
      body.push(paint(skirt, bottom));
    }

    // --- корпус
    var torso = new THREE.CylinderGeometry(0.19, 0.21, 0.5, 12);
    torso.translate(0, 1.12, 0);
    body.push(pattern ? paintPattern(torso, pattern, 2, 1) : paint(torso, top));
    var shoulders = new THREE.SphereGeometry(0.2, 12, 8);
    var msh = new THREE.Matrix4().makeScale(1.1, 0.6, 0.9);
    msh.setPosition(0, 1.35, 0);
    shoulders.applyMatrix4(msh);
    body.push(pattern ? paintPattern(shoulders, pattern, 2, 1) : paint(shoulders, top));
    ...
    for (var ax = -1; ax <= 1; ax += 2) {
      ...
      arm.applyMatrix4(ma);
      body.push(pattern ? paintPattern(arm, pattern, 1, 1) : paint(arm, robe || top));
      var hand = new THREE.SphereGeometry(0.06, 8, 6);
      hand.translate(ax * 0.28, 0.78, 0);
      body.push(paint(hand, skin));
    }

    // --- пояс: японский оби, марийский кушак, арабский поясок
    if (style === 'tokyo') {
      var obi = new THREE.CylinderGeometry(0.215, 0.215, 0.22, 14);
      obi.translate(0, 1.0, 0);
      body.push(paint(obi, kind === 'woman' ? 24 : 26));
      var bow = new THREE.BoxGeometry(0.2, 0.18, 0.14);
      bow.translate(0, 1.0, -0.22);
      body.push(paint(bow, kind === 'woman' ? 24 : 26));
    } else if (style === 'mari') {
      var sash = new THREE.CylinderGeometry(0.215, 0.215, 0.14, 14);
      sash.translate(0, 1.0, 0);
      body.push(paint(sash, 28));
      for (var tz = -1; tz <= 1; tz += 2) {      // подвески-монетки на поясе
        var tass = new THREE.BoxGeometry(0.05, 0.22, 0.05);
        tass.translate(tz * 0.1, 0.86, 0.2);
        body.push(paint(tass, 26));
      }
    }
    ...
    // --- головной убор по стране
    var cap = new THREE.SphereGeometry(0.132, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.55);
    cap.translate(0, 1.585, 0);

    if (style === 'dubai' && kind === 'man') {
      // гутра и чёрный обруч-игаль
      var cloth = new THREE.SphereGeometry(0.145, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.62);
      cloth.translate(0, 1.585, 0);
      body.push(paint(cloth, 16));
      var back = new THREE.BoxGeometry(0.24, 0.32, 0.1);
      back.translate(0, 1.44, -0.11);
      body.push(paint(back, 16));
      var ring = new THREE.TorusGeometry(0.13, 0.022, 6, 14);
      var mr = new THREE.Matrix4().makeRotationX(Math.PI / 2);
      mr.setPosition(0, 1.645, 0);
      ring.applyMatrix4(mr);
      body.push(paint(ring, 7));
    } else if (style === 'dubai') {
      // хиджаб: платок закрывает волосы и плечи
      var hij = new THREE.SphereGeometry(0.15, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.65);
      hij.translate(0, 1.585, 0);
      body.push(paint(hij, 17));
      var drape = new THREE.CylinderGeometry(0.16, 0.24, 0.36, 12);
      drape.translate(0, 1.34, -0.02);
      body.push(paint(drape, 17));
    } else if (style === 'moscow' && kind === 'man') {
      // шапка-ушанка
      var ush = new THREE.SphereGeometry(0.15, 12, 9, 0, Math.PI * 2, 0, Math.PI * 0.6);
      ush.translate(0, 1.6, 0);
      body.push(paint(ush, 23));
      for (var ex = -1; ex <= 1; ex += 2) {
        var ear = new THREE.BoxGeometry(0.07, 0.16, 0.13);
        ear.translate(ex * 0.145, 1.55, 0);
        body.push(paint(ear, 23));
      }
    } else if (style === 'moscow') {
      // пуховый платок
      var pl = new THREE.SphereGeometry(0.155, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.62);
      pl.translate(0, 1.585, 0);
      body.push(paintPattern(pl, 22, 2, 2));
      var knot = new THREE.SphereGeometry(0.075, 8, 6);
      knot.translate(0, 1.44, 0.09);
      body.push(paintPattern(knot, 22, 1, 1));
      var collar = new THREE.CylinderGeometry(0.21, 0.26, 0.16, 12);
      collar.translate(0, 1.38, 0);
      body.push(paint(collar, 23));
    } else if (style === 'mari') {
      if (kind === 'woman') {
        // шымакш: конусная шапочка с вышитым полотном на спине
        var cone = new THREE.ConeGeometry(0.11, 0.26, 10);
        var mc = new THREE.Matrix4().makeRotationX(-0.35);
        mc.setPosition(0, 1.74, -0.02);
        cone.applyMatrix4(mc);
        body.push(paintPattern(cone, 19, 1, 1));
        var tail = new THREE.BoxGeometry(0.14, 0.34, 0.06);
        tail.translate(0, 1.5, -0.13);
        body.push(paintPattern(tail, 19, 1, 2));
      } else {
        body.push(paint(cap, 16));               // белая холщовая шапочка
        var brim = new THREE.CylinderGeometry(0.175, 0.175, 0.03, 14);
        brim.translate(0, 1.63, 0);
        body.push(paint(brim, 16));
      }
    } else if (style === 'tokyo') {
      if (kind === 'woman') {
        var bun = new THREE.SphereGeometry(0.1, 10, 8);       // причёска с пучком
        bun.translate(0, 1.63, -0.11);
        body.push(paint(bun, 3));
        body.push(paint(cap, 3));
        var pin = new THREE.CylinderGeometry(0.012, 0.012, 0.22, 6);
        var mpn = new THREE.Matrix4().makeRotationZ(1.0);
        mpn.setPosition(0.02, 1.66, -0.09);
        pin.applyMatrix4(mpn);
        body.push(paint(pin, 26));
      } else {
        body.push(paint(cap, 3));
      }
    } else if (style === 'paris') {
      body.push(paint(cap, hair));
      var beret = new THREE.SphereGeometry(0.155, 12, 8, 0, Math.PI * 2, 0, Math.PI * 0.42);
      var mbe = new THREE.Matrix4().makeScale(1, 0.6, 1);
      mbe.setPosition(0.02, 1.63, -0.01);
      beret.applyMatrix4(mbe);
      body.push(paint(beret, pick([7, 24, 30])));
      var scarf = new THREE.TorusGeometry(0.115, 0.045, 6, 12);
      var msc = new THREE.Matrix4().makeRotationX(Math.PI / 2);
      msc.setPosition(0, 1.42, 0);
      scarf.applyMatrix4(msc);
      body.push(paint(scarf, pick([24, 9, 28])));
    } else if (style === 'swiss') {
      body.push(paint(cap, hair));
      // альпийская шляпа с полями и пёрышком
      var brim = new THREE.CylinderGeometry(0.24, 0.24, 0.035, 14);
      brim.translate(0, 1.63, 0);
      body.push(paint(brim, 27));
      var crown = new THREE.CylinderGeometry(0.13, 0.145, 0.16, 12);
      crown.translate(0, 1.71, 0);
      body.push(paint(crown, 27));
      var feather = new THREE.ConeGeometry(0.035, 0.3, 5);
      var mfe = new THREE.Matrix4().makeRotationZ(-0.5);
      mfe.setPosition(0.16, 1.8, -0.04);
      feather.applyMatrix4(mfe);
      body.push(paint(feather, 25));
      if (kind === 'woman') {                  // фартук дирндля
        var apron = new THREE.CylinderGeometry(0.2, 0.31, 0.46, 12);
        apron.translate(0, 0.86, 0.02);
        body.push(paint(apron, 29));
      } else {                                  // лямки на груди
        for (var bx2 = -1; bx2 <= 1; bx2 += 2) {
          var strap = new THREE.BoxGeometry(0.07, 0.5, 0.06);
          var mst = new THREE.Matrix4().makeRotationZ(bx2 * 0.12);
          mst.setPosition(bx2 * 0.09, 1.14, 0.19);
          strap.applyMatrix4(mst);
          body.push(paint(strap, 27));
        }
      }
    } else if (style === 'guard') {
    ...
    // сумка через плечо — у всех, кроме арабов в кандуре
    if (style !== 'dubai' && Math.random() < 0.45) {
      var bag = new THREE.BoxGeometry(0.22, 0.24, 0.1);
      bag.translate(0.24, 0.95, 0.05);
      body.push(paint(bag, pick([24, 25, 14])));
    }
    // француз с багетом
    if (style === 'paris' && Math.random() < 0.4) {
      var baguette = new THREE.CylinderGeometry(0.045, 0.045, 0.62, 8);
      var mbg = new THREE.Matrix4().makeRotationZ(0.5);
      mbg.setPosition(-0.28, 1.0, 0.12);
      baguette.applyMatrix4(mbg);
      body.push(paint(baguette, 29));
    }
  ...
  /* Луковка на барабане: сплюснутый шар с острым носиком и крестом. */
  function onionDome(x, y, z, r, domeGeos, crossGeos, drumGeos, drumH) {
    var drum = new THREE.CylinderGeometry(r * 0.72, r * 0.72, drumH, 14);
    drum.translate(x, y + drumH / 2, z);
    drumGeos.push(drum);
    var base = y + drumH;

    // сама луковица: профиль вращения, чтобы был характерный «пузырь с носиком»
    var pts = [];
    for (var i = 0; i <= 12; i++) {
      var t = i / 12;
      var rr = Math.sin(t * Math.PI * 0.92) * r * (1 - t * 0.15) + r * 0.12 * (1 - t);
      pts.push(new THREE.Vector2(Math.max(0.02, rr), t * r * 1.8));
    }
    var dome = new THREE.LatheGeometry(pts, 14);
    dome.translate(x, base, z);
    domeGeos.push(dome);

    var neck = new THREE.CylinderGeometry(r * 0.1, r * 0.14, r * 0.4, 8);
    neck.translate(x, base + r * 1.85, z);
    crossGeos.push(neck);
    var vert = new THREE.BoxGeometry(r * 0.06, r * 0.7, r * 0.06);
    vert.translate(x, base + r * 2.35, z);
    crossGeos.push(vert);
    var horiz = new THREE.BoxGeometry(r * 0.4, r * 0.06, r * 0.06);
    horiz.translate(x, base + r * 2.45, z);
    crossGeos.push(horiz);
  }

  /* Собор: белые стены, барабаны с луковками, апсиды и колокольня.
     style: 'blue' — синие купола со звёздами (Йошкар-Ола),
            'basil' — разноцветные (московский). */
  World.buildCathedral = function (group, x, z, angle, style) {
    var root = new THREE.Group();
    root.position.set(x, 0, z);
    root.rotation.y = angle || 0;
    group.add(root);

    var wallMat = new THREE.MeshLambertMaterial({ map: World.plasterTexture('#f2ece0') });
    var roofMat = new THREE.MeshLambertMaterial({ color: style === 'basil' ? 0x2f6c50 : 0x9a9c9f });
    var goldMat = new THREE.MeshPhongMaterial({ color: 0xd9a441, specular: 0xfff0c0, shininess: 80 });
    var domeColors = style === 'basil'
      ? [0x2f6c50, 0xc23b3b, 0x3f6fb5, 0xd9a441, 0x6f4a9c]
      : [0x2a4d8f, 0x2a4d8f, 0x2a4d8f, 0x2a4d8f, 0xd9a441];

    var walls = [], roofs = [], drums = [], crosses = [];
    var domesByColor = {};
    function domeBag(c) {
      if (!domesByColor[c]) domesByColor[c] = [];
      return domesByColor[c];
    }

    // основной четверик
    var body = new THREE.BoxGeometry(20, 15, 26);
    World.scaleUV(body, 3, 2);
    body.translate(0, 7.5, 0);
    walls.push(body);
    // алтарные апсиды
    for (var a = -1; a <= 1; a++) {
      var apse = new THREE.CylinderGeometry(3.4, 3.4, 11, 12, 1, false, -Math.PI / 2, Math.PI);
      apse.translate(a * 6.2, 5.5, -14.4);
      walls.push(apse);
    }
    // крыльцо
    var porch = new THREE.BoxGeometry(9, 7, 4);
    World.scaleUV(porch, 1, 1);
    porch.translate(0, 3.5, 15);
    walls.push(porch);

    // карниз-закомары
    var cornice = new THREE.BoxGeometry(21.4, 1.2, 27.4);
    cornice.translate(0, 15.4, 0);
    roofs.push(cornice);
    var roofSlab = new THREE.BoxGeometry(19, 1.6, 25);
    roofSlab.translate(0, 16.6, 0);
    roofs.push(roofSlab);

    // пять глав: центральная выше, четыре по углам
    onionDome(0, 17.4, 0, 3.4, domeBag(domeColors[4]), crosses, drums, 8);
    var corners = [[-6.4, -8], [6.4, -8], [-6.4, 8], [6.4, 8]];
    for (var i = 0; i < corners.length; i++) {
      onionDome(corners[i][0], 17.4, corners[i][1], 2.1,
        domeBag(domeColors[i % 4]), crosses, drums, 5);
    }

    // колокольня с шатром и главкой
    var tower = new THREE.BoxGeometry(7, 26, 7);
    World.scaleUV(tower, 1, 3);
    tower.translate(0, 13, 20);
    walls.push(tower);
    var belfry = new THREE.BoxGeometry(8, 6, 8);
    belfry.translate(0, 29, 20);
    walls.push(belfry);
    var tent = new THREE.ConeGeometry(6.2, 9, 4);
    tent.rotateY(Math.PI / 4);
    tent.translate(0, 36.5, 20);
    roofs.push(tent);
    onionDome(0, 41, 20, 1.5, domeBag(0xd9a441), crosses, drums, 2);

    root.add(new THREE.Mesh(World.mergeGeometries(walls), wallMat));
    root.add(new THREE.Mesh(World.mergeGeometries(roofs), roofMat));
    root.add(new THREE.Mesh(World.mergeGeometries(drums), wallMat));
    root.add(new THREE.Mesh(World.mergeGeometries(crosses), goldMat));
    Object.keys(domesByColor).forEach(function (c) {
      var mat = +c === 0xd9a441 ? goldMat
        : new THREE.MeshPhongMaterial({ color: +c, specular: 0x9fb6d8, shininess: 40 });
      root.add(new THREE.Mesh(World.mergeGeometries(domesByColor[c]), mat));
    });

    // ограда и ёлки вокруг
    var fence = [];
    for (var f = 0; f < 26; f++) {
      var ang = f / 26 * Math.PI * 2;
      var post = new THREE.BoxGeometry(0.5, 2.2, 0.5);
      post.translate(Math.cos(ang) * 26, 1.1, Math.sin(ang) * 26);
      fence.push(post);
    }
    root.add(new THREE.Mesh(World.mergeGeometries(fence),
  ...
  World.buildFerrisWheel = function (group, x, z, angle) {
    var root = new THREE.Group();
    root.position.set(x, 0, z);
    root.rotation.y = angle || 0;
    group.add(root);

    var steelMat = new THREE.MeshLambertMaterial({ color: 0xc9ced6 });
    var R = 24, hubY = 30;
    var legs = [];
    for (var s = -1; s <= 1; s += 2) {
      for (var k = -1; k <= 1; k += 2) {
        var leg = new THREE.CylinderGeometry(0.5, 0.8, hubY * 1.06, 8);
        var m = new THREE.Matrix4().makeRotationZ(s * 0.28);
        m.premultiply(new THREE.Matrix4().makeRotationX(k * 0.12));
        m.setPosition(s * hubY * 0.28, hubY / 2, k * 3.2);
        leg.applyMatrix4(m);
        legs.push(leg);
      }
    }
    var axle = new THREE.CylinderGeometry(1, 1, 8, 10);
    axle.rotateX(Math.PI / 2);
    axle.translate(0, hubY, 0);
    legs.push(axle);
    root.add(new THREE.Mesh(World.mergeGeometries(legs), steelMat));

    // само колесо крутится
    ...
    for (var i = 0; i < N; i++) {
      ...
      // спица от центра наружу
      var sp = new THREE.CylinderGeometry(0.16, 0.16, R, 6);
      sp.translate(0, R / 2, 0);
      sp.rotateZ(-a1);
      rim.push(sp);
      // сегмент обода
      var a2 = (i + 1) / N * Math.PI * 2;
      var p1 = new THREE.Vector3(Math.sin(a1) * R, Math.cos(a1) * R, 0);
      var p2 = new THREE.Vector3(Math.sin(a2) * R, Math.cos(a2) * R, 0);
      var len = p1.distanceTo(p2);
      var seg = new THREE.CylinderGeometry(0.22, 0.22, len * 1.02, 6);
      ...
      var mm = new THREE.Matrix4().makeRotationFromQuaternion(q);
      mm.setPosition(mid.x, mid.y, mid.z);
      seg.applyMatrix4(mm);
      rim.push(seg);
    }
    wheel.add(new THREE.Mesh(World.mergeGeometries(rim), steelMat));

    // кабинки: висят на ободе, поэтому каждая — своя группа
    var cabColors = [0xe14b4b, 0x3f7fd8, 0x39b06a, 0xf0c23c, 0x8a5cc4, 0xff8a3d];
    var cabs = [];
    for (var c = 0; c < N; c++) {
      var ac = c / N * Math.PI * 2;
      var pivot = new THREE.Group();
      pivot.position.set(Math.sin(ac) * R, Math.cos(ac) * R, 0);
      var cab = new THREE.Mesh(new THREE.BoxGeometry(2.6, 2.4, 3),
        new THREE.MeshLambertMaterial({ color: cabColors[c % cabColors.length] }));
      cab.position.y = -1.8;
      pivot.add(cab);
      wheel.add(pivot);
      cabs.push(pivot);
    ...
    root.userData.cabs = cabs;
    World.ferrisWheels.push(root);
    return root;
  };

  World.ferrisWheels = [];

  /* Живые детали города: колесо обозрения крутится, кабинки висят ровно. */
  World.updateLandmarks = function (dt) {
    for (var c = 0; c < World.cableCars.length; c++) {
      var cc = World.cableCars[c];
      cc.userData.phase = (cc.userData.phase + dt * 0.02) % 1;
      var cabs = cc.userData.cabs;
      for (var k = 0; k < cabs.length; k++) {
        var t = (cc.userData.phase + k / cabs.length) % 1;
        // половина кабинок едет вверх, половина вниз
        var up = k % 2 === 0 ? t : 1 - t;
        cabs[k].position.lerpVectors(cc.userData.from, cc.userData.to, up);
        cabs[k].position.z += (k % 2 === 0 ? 2.4 : -2.4);
        cabs[k].rotation.z = Math.sin(cc.userData.phase * 30 + k) * 0.05;
      }
    }
    for (var i = 0; i < World.ferrisWheels.length; i++) {
      var w = World.ferrisWheels[i];
      w.userData.wheel.rotation.z += dt * 0.09;
      var cabs = w.userData.cabs;
      for (var c = 0; c < cabs.length; c++) cabs[c].rotation.z = -w.userData.wheel.rotation.z;
    }
  };

  /* ---------- Японская пагода ---------- */

  World.buildPagoda = function (group, x, z, angle) {
    var root = new THREE.Group();
    root.position.set(x, 0, z);
    root.rotation.y = angle || 0;
    group.add(root);

    var redMat = new THREE.MeshLambertMaterial({ color: 0xb5342c });
    var roofMat = new THREE.MeshLambertMaterial({ color: 0x2f3a44 });
    var whiteMat = new THREE.MeshLambertMaterial({ color: 0xe8e2d4 });
    var goldMat = new THREE.MeshPhongMaterial({ color: 0xd9a441, specular: 0xfff0c0, shininess: 70 });
    ...
    for (var t = 0; t < 5; t++) {
      var w = 14 - t * 1.9;
      var h = 5 - t * 0.35;
      var body = new THREE.BoxGeometry(w, h, w);
      body.translate(0, y + h / 2, y * 0 + 0);
      red.push(body);
      // белые панели между столбами
      var panel = new THREE.BoxGeometry(w * 0.86, h * 0.6, w * 0.86);
      panel.translate(0, y + h / 2, 0);
      white.push(panel);

      // крыша с загнутыми углами: тонкая пирамида + карниз
      var eaveW = w + 7 - t * 0.8;
      var eave = new THREE.BoxGeometry(eaveW, 0.5, eaveW);
      eave.translate(0, y + h + 0.25, 0);
      roofs.push(eave);
      var pyr = new THREE.ConeGeometry(eaveW * 0.72, 2.6, 4);
      pyr.rotateY(Math.PI / 4);
      pyr.translate(0, y + h + 1.7, 0);
      roofs.push(pyr);
      // приподнятые уголки
      for (var cx = -1; cx <= 1; cx += 2) {
        for (var cz = -1; cz <= 1; cz += 2) {
          var tip = new THREE.ConeGeometry(0.55, 1.8, 4);
          var mt = new THREE.Matrix4().makeRotationX(cz * 0.5);
          mt.premultiply(new THREE.Matrix4().makeRotationZ(-cx * 0.5));
          mt.setPosition(cx * eaveW * 0.5, y + h + 1.1, cz * eaveW * 0.5);
          tip.applyMatrix4(mt);
          roofs.push(tip);
        }
      }
    ...
    // шпиль с кольцами
    var mast = new THREE.CylinderGeometry(0.3, 0.4, 9, 8);
    mast.translate(0, y + 4.5, 0);
    red.push(mast);
    var rings = [];
    for (var r = 0; r < 6; r++) {
      var ring = new THREE.TorusGeometry(0.9 - r * 0.1, 0.13, 6, 12);
      ring.rotateX(Math.PI / 2);
      ring.translate(0, y + 2 + r * 1.1, 0);
      rings.push(ring);
    }
    var ball = new THREE.SphereGeometry(0.8, 10, 8);
    ball.translate(0, y + 9.4, 0);
    rings.push(ball);

    root.add(new THREE.Mesh(World.mergeGeometries(red), redMat));
    root.add(new THREE.Mesh(World.mergeGeometries(white), whiteMat));
    root.add(new THREE.Mesh(World.mergeGeometries(roofs), roofMat));
    root.add(new THREE.Mesh(World.mergeGeometries(rings), goldMat));
    ...
    for (var g = 0; g < 2; g++) {
      var zz = 20 + g * 12;
      for (var px = -1; px <= 1; px += 2) {
        var post = new THREE.CylinderGeometry(0.5, 0.6, 8, 8);
        post.translate(px * 4, 4, zz);
        torii.push(post);
      }
      var top = new THREE.BoxGeometry(11, 0.8, 0.9);
      top.translate(0, 8.2, zz);
      torii.push(top);
      var top2 = new THREE.BoxGeometry(9, 0.6, 0.7);
      top2.translate(0, 7.1, zz);
      torii.push(top2);
    }
    root.add(new THREE.Mesh(World.mergeGeometries(torii), redMat));
    return root;
  };

  /* ---------- Триумфальная арка ---------- */

  World.buildArc = function (group, x, z, angle) {
    var root = new THREE.Group();
    root.position.set(x, 0, z);
    root.rotation.y = angle || 0;
    group.add(root);

    var stoneMat = new THREE.MeshLambertMaterial({ map: World.plasterTexture('#ded2b4') });
    var W = 44, H = 48, D = 22, PW = 13;
    var parts = [];

    // два пилона
    for (var s = -1; s <= 1; s += 2) {
      var pier = new THREE.BoxGeometry(PW, H * 0.72, D);
      World.scaleUV(pier, 2, 3);
      pier.translate(s * (W / 2 - PW / 2), H * 0.36, 0);
      parts.push(pier);
    }
    // перемычка над проёмом
    var lintel = new THREE.BoxGeometry(W, H * 0.28, D);
    World.scaleUV(lintel, 5, 1);
    lintel.translate(0, H * 0.86, 0);
    parts.push(lintel);
    // свод арки: ступеньками, чтобы читалась дуга
    for (var i = 0; i < 9; i++) {
      var t = (i + 0.5) / 9;
      var ax = (t - 0.5) * (W - PW * 2);
      var hh = Math.sqrt(Math.max(0, 1 - Math.pow((t - 0.5) * 2, 2))) * 8;
      var vou = new THREE.BoxGeometry((W - PW * 2) / 9 + 0.2, 8 - hh + 0.2, D);
      vou.translate(ax, H * 0.72 - (8 - hh) / 2 + 0.1, 0);
      parts.push(vou);
    }
    // карниз и аттик
    var cor = new THREE.BoxGeometry(W + 3, 2.4, D + 3);
    cor.translate(0, H - 1.2, 0);
    parts.push(cor);
    var attic = new THREE.BoxGeometry(W - 4, 4, D - 4);
    attic.translate(0, H + 2, 0);
    parts.push(attic);
    // рельефы по бокам проёма
    for (var r = -1; r <= 1; r += 2) {
      for (var q = -1; q <= 1; q += 2) {
        var relief = new THREE.BoxGeometry(7, 12, 0.8);
        relief.translate(r * (W / 2 - PW / 2), H * 0.42, q * (D / 2 + 0.3));
        parts.push(relief);
      }
    }
    root.add(new THREE.Mesh(World.mergeGeometries(parts), stoneMat));

    // площадь вокруг
    var plaza = new THREE.Mesh(new THREE.CircleGeometry(46, 32),
      new THREE.MeshLambertMaterial({ color: 0xb9b09a }));
  ...
  World.elkFurTexture = function () {
    return canvasTexture(128, 128, function (g, w, h) {
      g.fillStyle = '#5a4433';
      g.fillRect(0, 0, w, h);
      for (var i = 0; i < 2400; i++) {
        g.strokeStyle = Math.random() > 0.5 ? 'rgba(110,86,64,0.5)' : 'rgba(48,34,24,0.5)';
        g.lineWidth = 1;
        var x = Math.random() * w, y = Math.random() * h;
        g.beginPath(); g.moveTo(x, y); g.lineTo(x + 1.5, y + 4); g.stroke();
      }
      // светлая «седина» на холке
      var grd = g.createLinearGradient(0, 0, 0, h);
      grd.addColorStop(0, 'rgba(150,126,100,0.35)');
  ...
  /* Сохатый: горбатая холка, тяжёлая морда с горбинкой, серьга под горлом
     и широкие лопаты рогов. Ноги — отдельные группы, чтобы шагал. */
  World.buildElk = function () {
    var g = new THREE.Group();
    var fur = new THREE.MeshLambertMaterial({ map: World.elkFurTexture() });
    var dark = new THREE.MeshLambertMaterial({ color: 0x2f251c });
    var antlerMat = new THREE.MeshLambertMaterial({ color: 0xbaa77f });

    // ---- корпус
    var body = [];
    var barrel = new THREE.SphereGeometry(1, 16, 12);
    var mb = new THREE.Matrix4().makeScale(0.82, 0.92, 1.55);
    mb.setPosition(0, 1.75, -0.1);
    barrel.applyMatrix4(mb);
    body.push(barrel);
    // высокая холка — главная примета лося
    var withers = new THREE.SphereGeometry(0.7, 14, 10);
    var mw = new THREE.Matrix4().makeScale(1, 1.15, 1.1);
    mw.setPosition(0, 2.15, 0.55);
    withers.applyMatrix4(mw);
    body.push(withers);
    var rump = new THREE.SphereGeometry(0.62, 12, 10);
    rump.translate(0, 1.7, -1.45);
    body.push(rump);
    g.add(new THREE.Mesh(World.mergeGeometries(body), fur));
    ...
    g.add(head);
    var neckParts = [];
    var neck = new THREE.CylinderGeometry(0.34, 0.46, 1.15, 12);
    var mn = new THREE.Matrix4().makeRotationX(-0.55);
    mn.setPosition(0, 0.35, 0.25);
    neck.applyMatrix4(mn);
    neckParts.push(neck);
    var skull = new THREE.SphereGeometry(0.3, 12, 10);
    var ms = new THREE.Matrix4().makeScale(0.85, 0.95, 1.5);
    ms.setPosition(0, 0.82, 0.85);
    skull.applyMatrix4(ms);
    neckParts.push(skull);
    var muzzle = new THREE.SphereGeometry(0.26, 12, 10);       // тяжёлая горбоносая морда
    var mm = new THREE.Matrix4().makeScale(0.9, 0.95, 1.25);
    mm.setPosition(0, 0.72, 1.45);
    muzzle.applyMatrix4(mm);
    neckParts.push(muzzle);
    for (var e = -1; e <= 1; e += 2) {                          // уши-лопухи
      var ear = new THREE.SphereGeometry(0.16, 8, 6);
      var me = new THREE.Matrix4().makeScale(0.45, 1, 0.8);
      me.premultiply(new THREE.Matrix4().makeRotationZ(e * 0.5));
      me.setPosition(e * 0.28, 1.02, 0.66);
      ear.applyMatrix4(me);
      neckParts.push(ear);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(neckParts), fur));

    var darkBits = [];
    var nose = new THREE.SphereGeometry(0.13, 8, 6);
    nose.translate(0, 0.68, 1.72);
    darkBits.push(nose);
    for (var ey = -1; ey <= 1; ey += 2) {
      var eye = new THREE.SphereGeometry(0.055, 6, 6);
      eye.translate(ey * 0.21, 0.92, 1.06);
      darkBits.push(eye);
    }
    // «серьга» — кожаный подвес под горлом
    var bell = new THREE.SphereGeometry(0.14, 8, 8);
    var mbl = new THREE.Matrix4().makeScale(0.7, 1.7, 0.7);
    mbl.setPosition(0, 0.28, 0.72);
    bell.applyMatrix4(mbl);
    darkBits.push(bell);
    head.add(new THREE.Mesh(World.mergeGeometries(darkBits), dark));

    // ---- рога-лопаты
    var antlers = [];
    for (var s = -1; s <= 1; s += 2) {
      var stem = new THREE.CylinderGeometry(0.09, 0.12, 0.5, 7);
      var mst = new THREE.Matrix4().makeRotationZ(s * 0.9);
      mst.setPosition(s * 0.22, 1.1, 0.62);
      stem.applyMatrix4(mst);
      antlers.push(stem);
      // лопата
      var palm = new THREE.BoxGeometry(0.95, 0.1, 0.75);
      var mp = new THREE.Matrix4().makeRotationZ(s * 0.35);
      mp.premultiply(new THREE.Matrix4().makeRotationY(s * -0.3));
      mp.setPosition(s * 0.78, 1.32, 0.55);
      palm.applyMatrix4(mp);
      antlers.push(palm);
      // отростки по краю лопаты
      for (var t = 0; t < 4; t++) {
        var tine = new THREE.ConeGeometry(0.08, 0.42, 6);
        var mti = new THREE.Matrix4().makeRotationZ(s * (0.6 + t * 0.1));
        mti.premultiply(new THREE.Matrix4().makeRotationX(-0.25 + t * 0.2));
        mti.setPosition(s * (1.05 + t * 0.06), 1.44, 0.85 - t * 0.35);
        tine.applyMatrix4(mti);
        antlers.push(tine);
      }
    }
    head.add(new THREE.Mesh(World.mergeGeometries(antlers), antlerMat));

    // ---- длинные ноги с копытами
    var legPivots = [];
    var legSpots = [[0.42, 0.75], [-0.42, 0.75], [0.42, -1.15], [-0.42, -1.15]];
    for (var i = 0; i < legSpots.length; i++) {
      var pivot = new THREE.Group();
      pivot.position.set(legSpots[i][0], 1.55, legSpots[i][1]);
      var lp = [];
      var thigh = new THREE.CylinderGeometry(0.16, 0.12, 0.85, 8);
      thigh.translate(0, -0.42, 0);
      lp.push(thigh);
      var shin = new THREE.CylinderGeometry(0.1, 0.08, 0.72, 8);
      var msh2 = new THREE.Matrix4().makeRotationX(i < 2 ? 0.12 : -0.12);
      msh2.setPosition(0, -1.2, i < 2 ? -0.04 : 0.04);
      shin.applyMatrix4(msh2);
      lp.push(shin);
      pivot.add(new THREE.Mesh(World.mergeGeometries(lp), fur));
      var hoof = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.12, 0.18, 8), dark);
      hoof.position.y = -1.62;
      pivot.add(hoof);
      g.add(pivot);
      legPivots.push(pivot);
    }
    ...
    var tail = new THREE.Group();
    tail.position.set(0, 1.95, -1.9);
    var tailMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.04, 0.32, 6), fur);
    tailMesh.position.y = -0.16;
    tail.add(tailMesh);
    g.add(tail);

    var shadow = new THREE.Mesh(new THREE.PlaneGeometry(3, 4.2),
      new THREE.MeshBasicMaterial({ map: World.shadowTexture(), transparent: true, depthWrite: false }));
    shadow.rotation.x = -Math.PI / 2;
    shadow.position.y = 0.05;
    g.add(shadow);
    ...
    g.visible = false;
    g.userData = { legs: legPivots, blob: shadow, head: head, tail: tail, kind: 'elk' };
    return g;
  };

  /* ---------- Японский дракон ---------- */

  World.dragonScaleTexture = function () {
    return canvasTexture(128, 128, function (g, w, h) {
      g.fillStyle = '#2f7d5a';
      g.fillRect(0, 0, w, h);
      for (var row = 0; row < 8; row++) {
        for (var col = 0; col < 8; col++) {
          var x = col * 16 + (row % 2 ? 8 : 0), y = row * 16;
          g.fillStyle = 'rgba(30,92,66,0.85)';
          g.beginPath();
          g.arc(x + 8, y + 8, 8, Math.PI, 0);
          g.fill();
          g.fillStyle = 'rgba(140,214,170,0.35)';
          g.beginPath();
          g.arc(x + 8, y + 7, 5, Math.PI, 0);
          g.fill();
        }
      }
    }, 3, 2);
  ...
  /* Дракон: длинное змеиное тело из сегментов, рогатая голова с усами,
     когтистые лапки и грива. Извивается на ходу. */
  World.buildDragon = function () {
    var g = new THREE.Group();
    var skin = new THREE.MeshLambertMaterial({ map: World.dragonScaleTexture() });
    var belly = new THREE.MeshLambertMaterial({ color: 0xe8c96a });
    var redMat = new THREE.MeshLambertMaterial({ color: 0xc0392b });
    var goldMat = new THREE.MeshPhongMaterial({ color: 0xd9a441, specular: 0xfff0c0, shininess: 70 });

    // тело: цепочка сегментов, каждый качается сам — получается волна
    var segs = [];
    var parent = g;
    for (var i = 0; i < 10; i++) {
      var seg = new THREE.Group();
      seg.position.set(0, i === 0 ? 2.2 : 0, i === 0 ? 0 : -0.85);
      var r = 0.52 - i * 0.03;
      var tube = new THREE.Mesh(new THREE.SphereGeometry(r, 12, 10), skin);
      tube.scale.set(1, 0.92, 1.35);
      seg.add(tube);
      var scale = new THREE.Mesh(new THREE.ConeGeometry(r * 0.42, r * 0.9, 4), redMat);
      scale.position.y = r * 0.95;
      seg.add(scale);
      parent.add(seg);
      parent = seg;
      segs.push(seg);
    ...
    // хвостовой плавник
    var fin = new THREE.Mesh(new THREE.ConeGeometry(0.42, 1.2, 4), redMat);
    ...
    g.add(head);
    var skull = [];
    var dome = new THREE.SphereGeometry(0.55, 14, 10);
    var md = new THREE.Matrix4().makeScale(1, 0.85, 1.25);
    md.setPosition(0, 0, 0.1);
    dome.applyMatrix4(md);
    skull.push(dome);
    var snout = new THREE.SphereGeometry(0.36, 12, 8);
    var msn = new THREE.Matrix4().makeScale(0.95, 0.75, 1.5);
    msn.setPosition(0, -0.12, 0.85);
    snout.applyMatrix4(msn);
    skull.push(snout);
    head.add(new THREE.Mesh(World.mergeGeometries(skull), skin));

    var jaw = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.18, 0.9), belly);
    jaw.position.set(0, -0.34, 0.75);
    head.add(jaw);

    var horns = [];
    for (var s = -1; s <= 1; s += 2) {
      var horn = new THREE.ConeGeometry(0.1, 0.85, 7);
      var mh = new THREE.Matrix4().makeRotationZ(s * 0.55);
      mh.premultiply(new THREE.Matrix4().makeRotationX(-0.5));
      mh.setPosition(s * 0.28, 0.5, -0.15);
      horn.applyMatrix4(mh);
      horns.push(horn);
      // усы-вибриссы
      var whisker = new THREE.CylinderGeometry(0.035, 0.02, 1.5, 6);
      var mwh = new THREE.Matrix4().makeRotationZ(s * 0.35);
      mwh.premultiply(new THREE.Matrix4().makeRotationX(1.3));
      mwh.setPosition(s * 0.34, -0.12, 1.25);
      whisker.applyMatrix4(mwh);
      horns.push(whisker);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(horns), goldMat));

    var eyes = [];
    for (var ey = -1; ey <= 1; ey += 2) {
      var eye = new THREE.SphereGeometry(0.11, 8, 8);
      eye.translate(ey * 0.3, 0.14, 0.5);
      eyes.push(eye);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(eyes),
      new THREE.MeshLambertMaterial({ color: 0xffd23f, emissive: 0x6a4a00 })));

    // грива вокруг головы
    var mane = [];
    for (var m = 0; m < 9; m++) {
      var a = (m / 9) * Math.PI * 2;
      var tuft = new THREE.ConeGeometry(0.13, 0.6, 5);
      var mt = new THREE.Matrix4().makeRotationZ(Math.cos(a) * 0.9);
      mt.premultiply(new THREE.Matrix4().makeRotationX(Math.sin(a) * 0.9));
      mt.setPosition(Math.cos(a) * 0.42, Math.sin(a) * 0.42, -0.4);
      tuft.applyMatrix4(mt);
      mane.push(tuft);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(mane), redMat));

    // лапки с когтями
    var legPivots = [];
    var spots = [[0.55, 0.2], [-0.55, 0.2], [0.5, -2.2], [-0.5, -2.2]];
    for (var l = 0; l < spots.length; l++) {
      var pivot = new THREE.Group();
      pivot.position.set(spots[l][0], 2.0, spots[l][1]);
      var limb = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.11, 1.1, 8), skin);
      limb.position.y = -0.55;
      pivot.add(limb);
      var claw = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.34, 6), goldMat);
      claw.rotation.x = Math.PI;
      claw.position.set(0, -1.2, 0.1);
      pivot.add(claw);
      g.add(pivot);
      legPivots.push(pivot);
    }

    var shadow = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 7),
      new THREE.MeshBasicMaterial({ map: World.shadowTexture(), transparent: true, depthWrite: false }));
    shadow.rotation.x = -Math.PI / 2;
    shadow.position.y = 0.05;
    g.add(shadow);
    ...
    g.visible = false;
    g.userData = { legs: legPivots, blob: shadow, head: head, segments: segs, kind: 'dragon' };
    return g;
  };

  /* ---------- Гигантская улитка ---------- */

  World.shellTexture = function () {
    return canvasTexture(128, 128, function (g, w, h) {
      g.fillStyle = '#d8b072';
      g.fillRect(0, 0, w, h);
      for (var i = 0; i < 10; i++) {                  // полосы вдоль витка
        g.fillStyle = i % 2 ? 'rgba(140,88,44,0.55)' : 'rgba(240,214,168,0.5)';
        g.fillRect(0, i * 13, w, 6);
      }
      for (var k = 0; k < 600; k++) {
        g.fillStyle = 'rgba(110,70,36,0.16)';
        g.fillRect(Math.random() * w, Math.random() * h, 2, 1);
      }
    }, 1, 1);
  };

  /* Улитка размером с печку: витая раковина, мягкая нога, рожки с глазами. */
  World.buildSnail = function () {
    var g = new THREE.Group();
    var bodyMat = new THREE.MeshLambertMaterial({ color: 0xc9b48f });
    var shellMat = new THREE.MeshLambertMaterial({ map: World.shellTexture() });
    var wetMat = new THREE.MeshPhongMaterial({ color: 0xd8c6a0, specular: 0xffffff, shininess: 60 });

    // нога-подошва
    var foot = new THREE.Mesh(new THREE.SphereGeometry(1, 16, 10), wetMat);
    foot.scale.set(0.85, 0.42, 2.1);
    foot.position.y = 0.42;
    g.add(foot);

    // тело поднимается к голове
    var neck = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.62, 1.2, 12), bodyMat);
    neck.rotation.x = 0.6;
    neck.position.set(0, 0.95, 1.35);
    g.add(neck);

    var head = new THREE.Group();
    head.position.set(0, 1.35, 1.95);
    g.add(head);
    var skull = new THREE.Mesh(new THREE.SphereGeometry(0.5, 12, 10), bodyMat);
    skull.scale.set(1, 0.85, 1.15);
    head.add(skull);
    // рожки с глазами
    var horns = [];
    for (var s = -1; s <= 1; s += 2) {
      var stalk = new THREE.CylinderGeometry(0.07, 0.09, 1.15, 7);
      var m = new THREE.Matrix4().makeRotationZ(s * 0.32);
      m.premultiply(new THREE.Matrix4().makeRotationX(-0.2));
      m.setPosition(s * 0.28, 0.62, 0.12);
      stalk.applyMatrix4(m);
      horns.push(stalk);
      var eyeball = new THREE.SphereGeometry(0.14, 8, 8);
      eyeball.translate(s * 0.46, 1.16, 0.24);
      horns.push(eyeball);
    }
    // короткие нижние щупальца
    for (var t = -1; t <= 1; t += 2) {
      var tent = new THREE.CylinderGeometry(0.05, 0.07, 0.45, 6);
      var mt = new THREE.Matrix4().makeRotationX(1.2);
      mt.setPosition(t * 0.2, -0.18, 0.4);
      tent.applyMatrix4(mt);
      horns.push(tent);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(horns), bodyMat));
    var pupils = [];
    for (var p = -1; p <= 1; p += 2) {
      var pup = new THREE.SphereGeometry(0.07, 6, 6);
      pup.translate(p * 0.5, 1.19, 0.35);
      pupils.push(pup);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(pupils),
      new THREE.MeshLambertMaterial({ color: 0x18140f })));

    // раковина: спираль из шаров
    var shell = new THREE.Group();
    shell.position.set(0, 1.5, -0.5);
    shell.rotation.z = 0.25;
    g.add(shell);
    var whorl = [];
    for (var i = 0; i < 26; i++) {
      var t2 = i / 25;
      var a = t2 * Math.PI * 4.4;
      var rad = 1.55 * (1 - t2 * 0.86);
      var dist = 1.35 * (1 - t2 * 0.75);
      var ball = new THREE.SphereGeometry(rad, 12, 10);
      var mm2 = new THREE.Matrix4().makeScale(1, 1, 0.62);
      mm2.setPosition(Math.cos(a) * dist, Math.sin(a) * dist, t2 * 0.9 - 0.45);
      ball.applyMatrix4(mm2);
      whorl.push(ball);
    }
    shell.add(new THREE.Mesh(World.mergeGeometries(whorl), shellMat));

    var shadow = new THREE.Mesh(new THREE.PlaneGeometry(3.6, 5),
      new THREE.MeshBasicMaterial({ map: World.shadowTexture(), transparent: true, depthWrite: false }));
    shadow.rotation.x = -Math.PI / 2;
    shadow.position.y = 0.05;
    g.add(shadow);
    ...
    g.visible = false;
    g.userData = { legs: [], blob: shadow, head: head, shell: shell, kind: 'snail' };
    return g;
  };

  /* Зверь этого города. */
  World.buildBeast = function (kind) {
    if (kind === 'cow') return World.buildCow();
    if (kind === 'elk') return World.buildElk();
    if (kind === 'dragon') return World.buildDragon();
    if (kind === 'snail') return World.buildSnail();
    return World.buildCamel();
  };

  World.BEASTS = {
    camel: { name: 'верблюд', warn: 'Верблюд вышел на дорогу!', speed: [0.8, 0.5],
             hit: 'врезался в верблюда', chase: 'Верблюд заметил тебя и бежит следом!',
             attack: 'верблюд заплевал с ног до головы', bump: 'верблюд сбил тебя с ног',
             swing: 3, runSpeed: 3.4, color: 0x9fc46a },
    elk:   { name: 'лось', warn: 'Лось вышел на дорогу!', speed: [1.1, 0.7],
             hit: 'врезался в лося', chase: 'Лось наклонил рога и пошёл на тебя!',
             attack: 'лось поддел тебя рогами', bump: 'лось боднул тебя',
             swing: 3.6, runSpeed: 4.6, color: 0x8a6a3c },
    dragon:{ name: 'дракон', warn: 'Дракон опустился на дорогу!', speed: [1.6, 0.9],
             hit: 'врезался в дракона', chase: 'Дракон закружил над тобой!',
             attack: 'дракон обдал тебя огнём', bump: 'дракон сбил тебя хвостом',
             swing: 5, runSpeed: 5.2, color: 0xff7a2a },
    cow:   { name: 'корова', warn: 'Корова вышла на дорогу!', speed: [0.6, 0.4],
             hit: 'врезался в корову', chase: 'Корова обиделась и пошла на тебя!',
             attack: 'корова боднула и обслюнявила', bump: 'корова сбила тебя с ног',
             swing: 2.6, runSpeed: 3, color: 0xe8dcc8 },
    snail: { name: 'улитка', warn: 'Гигантская улитка выползла на дорогу!', speed: [0.28, 0.16],
  ...
  /* ---------- Альпы: горы, шале, канатка и Маттерхорн ---------- */

  World.rockTexture = function () {
    return canvasTexture(256, 256, function (g, w, h) {
      g.fillStyle = '#8f8c86';
      g.fillRect(0, 0, w, h);
      for (var i = 0; i < 5200; i++) {
        g.fillStyle = Math.random() > 0.5 ? 'rgba(158,154,146,0.5)' : 'rgba(84,80,74,0.5)';
        g.fillRect(Math.random() * w, Math.random() * h, 2.2, 2.2);
      }
      // сколы и трещины
      for (var k = 0; k < 70; k++) {
        g.strokeStyle = 'rgba(52,50,48,0.45)';
        g.lineWidth = 1 + Math.random() * 2;
        g.beginPath();
        var x = Math.random() * w, y = Math.random() * h;
        g.moveTo(x, y);
        g.lineTo(x + (Math.random() - 0.5) * 60, y + Math.random() * 50);
        g.stroke();
      }
      // снег в верхней части тайла — он ляжет на вершины
      var grd = g.createLinearGradient(0, 0, 0, h);
      grd.addColorStop(0, 'rgba(255,255,255,0.98)');
      grd.addColorStop(0.34, 'rgba(255,255,255,0.9)');
  ...
  /* Кольцо заснеженных вершин вокруг трассы. */
  World.buildMountains = function (group, track) {
    var mat = new THREE.MeshLambertMaterial({ map: World.rockTexture() });
    var geos = [];
    // центр и габарит трассы: горы ставим заведомо за её пределами,
    // иначе склон накроет дорогу
    var center = new THREE.Vector3(0, 0, 0);
    for (var c = 0; c < track.samples.length; c++) center.add(track.samples[c]);
    center.multiplyScalar(1 / track.samples.length);
    center.y = 0;
    var maxR = 0;
    for (var c2 = 0; c2 < track.samples.length; c2++) {
      var sp = track.samples[c2];
      var dx = sp.x - center.x, dz = sp.z - center.z;
      maxR = Math.max(maxR, Math.sqrt(dx * dx + dz * dz));
    }
    for (var i = 0; i < 26; i++) {
      var a = i / 26 * Math.PI * 2 + Math.random() * 0.12;
      var hgt = 240 + Math.random() * 380;
      var base = hgt * (0.55 + Math.random() * 0.3);
      var rad = maxR + base + 110 + Math.random() * 420;
      var x = center.x + Math.cos(a) * rad;
      var z = center.z + Math.sin(a) * rad;
      var peak = new THREE.ConeGeometry(base, hgt, 7 + Math.floor(Math.random() * 3));
      // чуть скошенная вершина: гора перестаёт быть идеальным конусом
      var pos = peak.attributes.position;
      for (var v = 0; v < pos.count; v++) {
        var py = pos.getY(v);
        var k = (py + hgt / 2) / hgt;
        pos.setX(v, pos.getX(v) * (1 + Math.sin(k * 5 + i) * 0.12));
        pos.setZ(v, pos.getZ(v) * (1 + Math.cos(k * 4 + i) * 0.12));
      }
      peak.computeVertexNormals();
      // текстура: снег наверху, поэтому растягиваем её по высоте один раз
      World.scaleUV(peak, 3, 1);
      peak.translate(x, hgt / 2 - 12, z);
      geos.push(peak);
    }
    group.add(new THREE.Mesh(World.mergeGeometries(geos), mat));

    // валуны на лугу вдоль серпантина: лежат на земле, чуть вросшие
    var near = [];
    for (var s = 0; s < 70; s++) {
      var d = track.length * s / 70 + 7;
      var p = track.pointAt(d);
      var n = track.sideAt(d);
      var sign = (s % 2 === 0) ? 1 : -1;
      var off = World.roadFootprint(track, d) + 5 + Math.random() * 26;
      var rp = p.clone().addScaledVector(n, sign * off);
      var rh = 2.2 + Math.random() * 5;
      var rock = new THREE.DodecahedronGeometry(rh, 0);
      var m = new THREE.Matrix4().makeScale(1 + Math.random() * 0.5, 0.7 + Math.random() * 0.3,
        1 + Math.random() * 0.5);
      m.premultiply(new THREE.Matrix4().makeRotationY(Math.random() * Math.PI));
      m.setPosition(rp.x, rh * 0.35, rp.z);
      rock.applyMatrix4(m);
      near.push(rock);
    }
    group.add(new THREE.Mesh(World.mergeGeometries(near),
      new THREE.MeshLambertMaterial({ color: 0x8d8a84 })));
  };

  /* Маттерхорн: узнаваемая кривая пирамида со снежной шапкой. */
  World.buildMatterhorn = function (group, x, z) {
    var root = new THREE.Group();
    root.position.set(x, 0, z);
    group.add(root);

    var H = 620;
    var rock = new THREE.ConeGeometry(220, H, 4, 6);
    rock.rotateY(Math.PI / 4);
    var pos = rock.attributes.position;
    for (var i = 0; i < pos.count; i++) {
      var y = pos.getY(i);
      var k = (y + H / 2) / H;                 // 0 — низ, 1 — вершина
      // характерный «крюк» вершины и вогнутые грани
      var bend = Math.pow(k, 3) * 60;
      var pinch = 1 - Math.pow(k, 1.35) * 0.12;
      pos.setX(i, pos.getX(i) * pinch + bend);
      pos.setZ(i, pos.getZ(i) * pinch - bend * 0.35);
    }
    rock.computeVertexNormals();
    World.scaleUV(rock, 2, 1);
    rock.translate(0, H / 2 - 20, 0);
    root.add(new THREE.Mesh(rock, new THREE.MeshLambertMaterial({ map: World.rockTexture() })));

    // подножие: пологая осыпь
    var skirt = new THREE.ConeGeometry(420, 150, 9);
    skirt.translate(0, 55, 0);
    root.add(new THREE.Mesh(skirt, new THREE.MeshLambertMaterial({ color: 0x7f8a72 })));
    return root;
  };
  ...
  World.buildChalet = function (group, x, z, angle, scale) {
    ...
    var wallMat = new THREE.MeshLambertMaterial({ map: World.plasterTexture('#f2ece0') });
    var woodMat = new THREE.MeshLambertMaterial({ color: 0x6b4a2a });
    var roofMat = new THREE.MeshLambertMaterial({ color: 0x4a423a });
    var snowMat = new THREE.MeshLambertMaterial({ color: 0xf4f7fa });

    var W = 26, D = 16, Hf = 12;
    var walls = [];
    var body = new THREE.BoxGeometry(W, Hf, D);
    World.scaleUV(body, 3, 2);
    body.translate(0, Hf / 2, 0);
    walls.push(body);
    root.add(new THREE.Mesh(World.mergeGeometries(walls), wallMat));

    // двускатная крыша с большим свесом
    var wood = [];
    var roof = new THREE.CylinderGeometry(D * 0.62, D * 0.62, W + 7, 3, 1);
    roof.rotateY(Math.PI / 6);
    roof.rotateZ(Math.PI / 2);
    roof.translate(0, Hf + D * 0.3 - 1.6, 0);
    var roofMesh = new THREE.Mesh(roof, roofMat);
    root.add(roofMesh);
    // снег на крыше
    var snow = new THREE.CylinderGeometry(D * 0.64, D * 0.64, W + 7.4, 3, 1, false, 0, Math.PI);
    snow.rotateY(Math.PI / 6);
    snow.rotateZ(Math.PI / 2);
    snow.translate(0, Hf + D * 0.3 - 1.3, 0);
    root.add(new THREE.Mesh(snow, snowMat));

    // балконы в два яруса
    for (var lvl = 0; lvl < 2; lvl++) {
      var y = 4.4 + lvl * 4.2;
      var deck = new THREE.BoxGeometry(W + 2.4, 0.4, 2.2);
      deck.translate(0, y, D / 2 + 1);
      wood.push(deck);
      var rail = new THREE.BoxGeometry(W + 2.4, 0.3, 0.25);
      rail.translate(0, y + 1.1, D / 2 + 2);
      wood.push(rail);
      for (var b = -6; b <= 6; b++) {
        var baluster = new THREE.BoxGeometry(0.22, 1.1, 0.22);
        baluster.translate(b * 2, y + 0.6, D / 2 + 2);
        wood.push(baluster);
      }
    }
    // поленница у стены
    for (var lg = 0; lg < 18; lg++) {
      var log = new THREE.CylinderGeometry(0.3, 0.3, 3.2, 7);
      log.rotateZ(Math.PI / 2);
      log.translate(-W / 2 - 2, 0.4 + Math.floor(lg / 6) * 0.62, -3 + (lg % 6) * 0.64);
      wood.push(log);
    }
    root.add(new THREE.Mesh(World.mergeGeometries(wood), woodMat));
    return root;
  };
  ...
  World.buildCableCar = function (group, x, z, angle) {
    ...
    var cabinMat = new THREE.MeshLambertMaterial({ color: 0xd8443c });

    var LOW = 26, HIGH = 150, RUN = 320;
    var parts = [];
    function tower(px, py, hgt) {
      for (var s = -1; s <= 1; s += 2) {
        for (var k = -1; k <= 1; k += 2) {
          var leg = new THREE.CylinderGeometry(0.5, 0.9, hgt, 6);
          var m = new THREE.Matrix4().makeRotationZ(s * 0.06);
          m.setPosition(px + s * 2.6, py + hgt / 2, k * 2.6);
          leg.applyMatrix4(m);
          parts.push(leg);
        }
      }
      var top = new THREE.BoxGeometry(8, 1.2, 7);
      top.translate(px, py + hgt, 0);
      parts.push(top);
    }
    tower(0, 0, LOW);
    tower(RUN, HIGH - 30, 30);

    // трос: длинный тонкий цилиндр от опоры к опоре
    var a = new THREE.Vector3(0, LOW, 0);
    var b = new THREE.Vector3(RUN, HIGH, 0);
    var len = a.distanceTo(b);
    for (var c = -1; c <= 1; c += 2) {
      var rope = new THREE.CylinderGeometry(0.12, 0.12, len, 5);
      var dir = b.clone().sub(a).normalize();
      var q = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), dir);
      var mm = new THREE.Matrix4().makeRotationFromQuaternion(q);
      var mid = a.clone().add(b).multiplyScalar(0.5);
      mm.setPosition(mid.x, mid.y, mid.z + c * 2.4);
      rope.applyMatrix4(mm);
      parts.push(rope);
    }
    root.add(new THREE.Mesh(World.mergeGeometries(parts), steel));

    var cabs = [];
    for (var i = 0; i < 4; i++) {
      var cab = new THREE.Group();
      var box = new THREE.Mesh(new THREE.BoxGeometry(3.4, 3, 3.4), cabinMat);
      box.position.y = -2.6;
      cab.add(box);
      var glass = new THREE.Mesh(new THREE.BoxGeometry(3.5, 1.4, 3.5),
        new THREE.MeshPhongMaterial({ color: 0x2b3644, specular: 0xcfe4ff, shininess: 80 }));
      glass.position.y = -2.2;
      cab.add(glass);
      var arm = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.14, 1.6, 6), steel);
      arm.position.y = -0.7;
      cab.add(arm);
      root.add(cab);
      cabs.push(cab);
    }
    root.userData = { cabs: cabs, from: a, to: b, phase: 0 };
    World.cableCars.push(root);
    return root;
  };
  ...
  World.cowHideTexture = function () {
    return canvasTexture(128, 128, function (g, w, h) {
      g.fillStyle = '#f2ece2';
      g.fillRect(0, 0, w, h);
      g.fillStyle = '#4a3a2c';
      for (var i = 0; i < 9; i++) {                 // тёмные пятна
        var x = Math.random() * w, y = Math.random() * h;
        g.beginPath();
        g.moveTo(x, y);
        for (var k = 0; k < 7; k++) {
  ...
  /* Швейцарская бурёнка: пятнистая, с рогами и колокольчиком. */
  World.buildCow = function () {
    var g = new THREE.Group();
    var hide = new THREE.MeshLambertMaterial({ map: World.cowHideTexture() });
    var dark = new THREE.MeshLambertMaterial({ color: 0x3a2d22 });
    var pink = new THREE.MeshLambertMaterial({ color: 0xdba9a0 });
    var bellMat = new THREE.MeshPhongMaterial({ color: 0xd9a441, specular: 0xfff0c0, shininess: 80 });

    // корпус
    var body = [];
    var barrel = new THREE.SphereGeometry(1, 16, 12);
    var mb = new THREE.Matrix4().makeScale(0.85, 0.85, 1.5);
    mb.setPosition(0, 1.5, 0);
    barrel.applyMatrix4(mb);
    body.push(barrel);
    var chest = new THREE.SphereGeometry(0.78, 12, 10);
    chest.translate(0, 1.55, 1.05);
    body.push(chest);
    g.add(new THREE.Mesh(World.mergeGeometries(body), hide));

    // голова
    var head = new THREE.Group();
    head.position.set(0, 1.9, 1.75);
    g.add(head);
    var skull = [];
    var neck = new THREE.CylinderGeometry(0.32, 0.42, 0.7, 10);
    var mn = new THREE.Matrix4().makeRotationX(-0.7);
    mn.setPosition(0, -0.12, -0.35);
    neck.applyMatrix4(mn);
    skull.push(neck);
    var face = new THREE.SphereGeometry(0.34, 12, 10);
    var mf = new THREE.Matrix4().makeScale(0.9, 0.9, 1.3);
    mf.setPosition(0, 0.15, 0.3);
    face.applyMatrix4(mf);
    skull.push(face);
    for (var e = -1; e <= 1; e += 2) {
      var ear = new THREE.SphereGeometry(0.15, 8, 6);
      var me = new THREE.Matrix4().makeScale(0.5, 0.9, 1);
      me.premultiply(new THREE.Matrix4().makeRotationZ(e * 0.8));
      me.setPosition(e * 0.35, 0.28, 0.1);
      ear.applyMatrix4(me);
      skull.push(ear);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(skull), hide));

    var muzzle = new THREE.Mesh(new THREE.SphereGeometry(0.22, 10, 8), pink);
    ...
    for (var hx = -1; hx <= 1; hx += 2) {
      var horn = new THREE.CylinderGeometry(0.05, 0.09, 0.5, 6);
      var mh = new THREE.Matrix4().makeRotationZ(hx * 1.1);
      mh.setPosition(hx * 0.3, 0.42, 0.05);
      horn.applyMatrix4(mh);
      horns.push(horn);
      var tip = new THREE.ConeGeometry(0.05, 0.24, 6);
      var mt = new THREE.Matrix4().makeRotationZ(hx * 0.6);
      mt.setPosition(hx * 0.52, 0.56, 0.05);
      tip.applyMatrix4(mt);
      horns.push(tip);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(horns),
      new THREE.MeshLambertMaterial({ color: 0xe0d6c0 })));

    var eyes = [];
    for (var ey = -1; ey <= 1; ey += 2) {
      var eye = new THREE.SphereGeometry(0.07, 8, 6);
      eye.translate(ey * 0.22, 0.26, 0.42);
      eyes.push(eye);
    }
    head.add(new THREE.Mesh(World.mergeGeometries(eyes), dark));

    // колокольчик на ремне
    var collar = new THREE.Mesh(new THREE.TorusGeometry(0.36, 0.07, 6, 14),
      new THREE.MeshLambertMaterial({ color: 0x8f3a2c }));
    collar.rotation.x = Math.PI / 2 - 0.5;
    collar.position.set(0, 1.62, 1.35);
    g.add(collar);
    var bell = new THREE.Group();
    bell.position.set(0, 1.32, 1.5);
    var cup = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.2, 0.3, 8), bellMat);
    bell.add(cup);
    var knob = new THREE.Mesh(new THREE.SphereGeometry(0.07, 8, 6), bellMat);
    ...
    // ноги
    var legPivots = [];
    var spots = [[0.42, 0.75], [-0.42, 0.75], [0.42, -0.85], [-0.42, -0.85]];
    for (var i = 0; i < spots.length; i++) {
      var pivot = new THREE.Group();
      pivot.position.set(spots[i][0], 1.35, spots[i][1]);
      var leg = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.11, 1.3, 8), hide);
      leg.position.y = -0.65;
      pivot.add(leg);
      var hoof = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.14, 0.18, 8), dark);
      hoof.position.y = -1.35;
      pivot.add(hoof);
      g.add(pivot);
      legPivots.push(pivot);
    }
    ...
    // вымя и хвост
    var udder = new THREE.Mesh(new THREE.SphereGeometry(0.3, 10, 8), pink);
    udder.scale.set(1, 0.7, 1.1);
    udder.position.set(0, 0.95, -0.5);
    g.add(udder);
    var tail = new THREE.Group();
    tail.position.set(0, 1.75, -1.45);
    var tailMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.04, 0.9, 6), hide);
    tailMesh.position.y = -0.45;
    tail.add(tailMesh);
    var tuft = new THREE.Mesh(new THREE.SphereGeometry(0.11, 8, 6), dark);
    tuft.position.y = -0.92;
    tail.add(tuft);
    g.add(tail);

    var shadow = new THREE.Mesh(new THREE.PlaneGeometry(2.6, 4),
      new THREE.MeshBasicMaterial({ map: World.shadowTexture(), transparent: true, depthWrite: false }));
    shadow.rotation.x = -Math.PI / 2;
    shadow.position.y = 0.05;
    g.add(shadow);
