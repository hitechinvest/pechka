...
(function (global) {
  ...
  World.skyTexture = function () {
    return canvasTexture(1024, 512, function (g, w, h) {
      ...
      grd.addColorStop(0.00, '#0b3372');
      grd.addColorStop(0.18, '#245fa8');
      grd.addColorStop(0.36, '#4a8cc9');
      grd.addColorStop(0.52, '#84badb');
      grd.addColorStop(0.66, '#c2d3d3');
      grd.addColorStop(0.78, '#e0caa0');
      grd.addColorStop(0.90, '#dcb379');
      grd.addColorStop(1.00, '#c4945c');
      g.fillStyle = grd;
      g.fillRect(0, 0, w, h);
      ...
      for (var i = 0; i < 90; i++) {
        ...
        var alpha = 0.05 + Math.random() * 0.16;
      ...
      haze.addColorStop(0, 'rgba(226,206,170,0)');
      haze.addColorStop(0.6, 'rgba(226,206,170,0.24)');
      haze.addColorStop(1, 'rgba(214,182,133,0.42)');
      g.fillStyle = haze;
  ...
  World.shoulderMesh = function (track) {
    var mat = new THREE.MeshLambertMaterial({ color: 0xbda574 });
  ...
  World.embankmentMesh = function (track) {
    ...
    geo.setIndex(idx);
    geo.computeVertexNormals();
    return new THREE.Mesh(geo, new THREE.MeshLambertMaterial({ color: 0xc7ab7b }));
  };
  ...
  World.buildCity = function (track, scene) {
    ...
    // небо: панорама на сфере, её же подкрашиваем в бурю
    var sky = new THREE.Mesh(
      new THREE.SphereGeometry(1800, 32, 20),
      new THREE.MeshBasicMaterial({ map: World.skyTexture(), side: THREE.BackSide,
    ...
    var STYLES = ['glass', 'sand', 'white', 'stripe'];
    for (var b2 = 0; b2 < BUCKETS; b2++) {
      ...
      var glassy = STYLES[b2 % STYLES.length] !== 'sand';
      var mat = glassy
    ...
    // Дворец шейха на дальней стороне круга
    var pd = track.length * 0.52;
    var pp = track.pointAt(pd);
    var pn = track.sideAt(pd);
    var pt = track.tangentAt(pd);
    var palace = new THREE.Vector3(pp.x, 0, pp.z).addScaledVector(pn, 105);
    World.buildPalace(group, palace.x, palace.z, Math.atan2(-pn.x, -pn.z));
  ...
  World.buildPalace = function (group, x, z, angle) {
    ...
    for (var i = 0; i < 12; i++) {
      ...
      World.palmGeometry(x + Math.cos(a) * 50, z + Math.sin(a) * 50, trunks, leaves);
    }
    group.add(new THREE.Mesh(World.mergeGeometries(trunks),
      new THREE.MeshLambertMaterial({ color: 0x8a6a44 })));
    group.add(new THREE.Mesh(World.mergeGeometries(leaves),
